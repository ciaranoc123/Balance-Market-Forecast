...
def create_model():
            ...
            for i in range(1):
                nn.add(Dense(112, input_shape=i_shape, activation='sigmoid'))
                
            for i in range(1):
                nn.add(Dense(160, activation='tanh'))
   
            for i in range(1):
                nn.add(Dense(80, activation='tanh'))
            
            nn.add(Dropout(0.155556, seed=123))
            
            for i in range(1):
                nn.add(Dense(48, activation=LeakyReLU))
                
            nn.add(Dense(80, activation='relu'))
            nn.add(Dense(16))
            opt = Adam(lr =0.006733)
...
rolling_walk_forward_validation(model=mmo, data=dat, start_time='6/1/2020 00:00', end_time='9/1/2020  10:00',                                targets=Y.columns.values.tolist(), training_days=-210,
