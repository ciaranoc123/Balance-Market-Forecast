...
def create_model():
            ...
            for i in range(2):
                nn.add(Dense(112, input_shape=i_shape, activation='tanh'))
            ...
            nn.add(Dropout(0.200, seed=123))
            
            for i in range(2):
                nn.add(Dense(64, activation='relu'))
                
            nn.add(Dense(48, activation='tanh'))
            nn.add(Dense(16))
...
mmo = KerasRegressor(build_fn=create_model, epochs=200, batch_size=32, verbose=2, callbacks=[es])

rolling_walk_forward_validation(model=mmo, data=dat, start_time='3/1/2021 00:00', end_time='6/1/2021  00:00',
                                targets=Y.columns.values.tolist(), training_days=-60,
                                path="/home/coconnor/BM_results_SH_18-19(60 days)")