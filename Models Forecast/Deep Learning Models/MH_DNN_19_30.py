...
def create_model():    
            ...
            dense1 = LSTM(16, return_sequences=True, activation=LeakyReLU, input_shape=i_shape_lstm)(visible1)
            dense2 = LSTM(64, return_sequences=True, activation='tanh', input_shape=i_shape_lstm)(dense1)
            dense3 = LSTM(16, return_sequences=True, activation='sigmoid', input_shape=i_shape_lstm)(dense2)
            do_lstm = Dropout(0.088889, seed=123)(dense3)
            dense4 = LSTM(128)(do_lstm)
            flat1 = Flatten()(dense4)    
    
            visible2 = Input(shape=(i_shape_ffnn))        
            dense5 = Dense(128, activation=LeakyReLU)(visible2)        
            dense6 = Dense(192, activation='tanh')(dense5)
            dense7 = Dense(256, activation=LeakyReLU)(dense6)        
            do_ffnn = Dropout(0.177778, seed=123)(dense7)
            dense8 = Dense(256, activation=LeakyReLU)(do_ffnn)
            flat2 = Flatten()(dense8)
    
            merged = concatenate([flat1, flat2])
            dense_f = Dense(32, activation='sigmoid')(merged)        
...
rolling_walk_forward_validation(model=mmo, data=dat, start_time='3/1/2021 00:00', end_time='6/1/2021  00:00',
                                targets=Y.columns.values.tolist(), training_days=-30, path="/home/ciaranoconnor123/BM_results_MH_19_30")