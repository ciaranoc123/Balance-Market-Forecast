...
def create_model():    
            ...
            dense1 = LSTM(32, return_sequences= True, activation='tanh' , input_shape=i_shape_lstm)(visible1)
            dense2 = LSTM(32, return_sequences= True, activation='tanh' , input_shape=i_shape_lstm)(dense1)
            do_lstm = Dropout(0.022222, seed=123)(dense2)
            dense3 = LSTM(32)(do_lstm)
            flat1 = Flatten()(dense3)    
    
            visible2 = Input(shape=(i_shape_ffnn))        
            dense5 = Dense(192, activation='relu')(visible2)        
            dense6 = Dense(192, activation=LeakyReLU)(dense5)
            do_ffnn = Dropout(0.000000, seed=123)(dense6)
            dense7 = Dense(192, activation='relu')(do_ffnn)
            flat2 = Flatten()(dense7)
    
            merged = concatenate([flat1, flat2])
            dense_f = Dense(256, activation='')(merged)        
            outputs = Dense(16)(dense_f)

            model = Model(inputs=[visible1, visible2], outputs=outputs)    
            opt = Adam(lr= 0.011156)
...
mmo = KerasRegressor(build_fn=create_model, epochs=500, batch_size=32, verbose=2, callbacks=[es])

rolling_walk_forward_validation(model=mmo, data=dat, start_time='6/1/2020 00:00', end_time='6/7/2020  00:00',
                                targets=Y.columns.values.tolist(), training_days=-210,
                                path="/home/coconnor/BM_results_MH_1-10")