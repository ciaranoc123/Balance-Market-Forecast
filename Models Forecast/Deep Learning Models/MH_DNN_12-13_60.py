...
def create_model():    
            ...
            dense1 = LSTM(192, return_sequences= True, activation=LeakyReLU, input_shape=i_shape_lstm)(visible1)
            dense2 = LSTM(48, return_sequences= True, activation='tanh' , input_shape=i_shape_lstm)(dense1)
            dense3 = LSTM(144, return_sequences= True, activation='relu' , input_shape=i_shape_lstm)(dense2)
            do_lstm = Dropout(0.111111, seed=123)(dense3)
            dense4 = LSTM(160)(do_lstm)
            flat1 = Flatten()(dense4)    
    
            visible2 = Input(shape=(i_shape_ffnn))        
            dense5 = Dense(48, activation='tanh')(visible2)        
            dense6 = Dense(224, activation=LeakyReLU)(dense5)
            dense7 = Dense(208, activation='sigmoid')(dense6)        
            do_ffnn = Dropout(0.111111, seed=123)(dense7)
            dense8 = Dense(176, activation='relu')(do_ffnn)
            flat2 = Flatten()(dense8)
    
            merged = concatenate([flat1, flat2])
            dense_f = Dense(160, activation='sigmoid')(merged)        
...
rolling_walk_forward_validation(model=mmo, data=dat, start_time='9/1/2020 00:00', end_time='12/1/2020  00:00',
                                ...
                                path="/home/coconnor/BM_results_MH_12-13_60")