...
def create_model():
            ...
            for i in range(1):
                nn.add(Dense(96, input_shape=i_shape, activation=LeakyReLU))
            ...
            nn.add(Dropout(0.13333, seed=123))
            
            for i in range(2):
                nn.add(Dense(144, activation=LeakyReLU))
                
            nn.add(Dense(16, activation=LeakyReLU))
            nn.add(Dense(16))
            opt = Adam(lr = 0.000100 )
...
rolling_walk_forward_validation(model=mmo, data=dat, start_time='3/1/2021 00:00', end_time='6/1/2021  00:00',
                                targets=Y.columns.values.tolist(), training_days=-360,
                                path="/home/coconnor/BM_results_SH_1-19")
