...
def create_model():    
            ...
            dense2 = LSTM(128, return_sequences= True, activation='relu' , input_shape=i_shape_lstm)(dense1)
            do_lstm = Dropout(0.111111, seed=123)(dense2)
            dense3 = LSTM(128)(do_lstm)
            flat1 = Flatten()(dense3)    
    
            visible2 = Input(shape=(i_shape_ffnn))        
            dense5 = Dense(128, activation='relu')(visible2)        
            dense6 = Dense(128, activation=LeakyReLU)(dense5)
            do_ffnn = Dropout(0.088889, seed=123)(dense6)
            dense7 = Dense(128, activation='relu')(do_ffnn)
            flat2 = Flatten()(dense7)
    
            merged = concatenate([flat1, flat2])
            dense_f = Dense(32, activation=LeakyReLU)(merged)        
...
mmo = KerasRegressor(build_fn=create_model, epochs=300, batch_size=8, verbose=2, callbacks=[es])
...
rolling_walk_forward_validation(model=mmo, data=dat, start_time='12/1/2020 00:00', end_time='3/1/2021  00:00',
                                ...
                                path="/home/coconnor/BM_results_MH_1-16")